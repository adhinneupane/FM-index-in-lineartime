...
def getSuffixArray(sortedOneTwo, sortedZero):
    ...
    for i in range(len(sortedOneTwo)+len(sortedZero)):
        if cOneTwo >= len(sortedOneTwo):
            suffixArray.append(int(sortedZero[cZero].split(":")[1]))
            SA.append(sortedZero[cZero])
            cZero += 1
        elif cZero >= len(sortedZero):
            suffixArray.append(int(sortedOneTwo[cOneTwo].split(":")[1]))
...
def main():
    ...
    for seq in sequence:
        ...
        firstIndexArray = [seq[i] for i in suffixArray]
        allArrays.append(suffixArray)
        firstIndexArrays.append(firstIndexArray)
        # ------
